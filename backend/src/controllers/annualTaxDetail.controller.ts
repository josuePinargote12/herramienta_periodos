...
export async function exportAnnualDetailPdf(req: Request, res: Response) {
  try {
    ...
    detail.months.forEach(row => drawRow([row.name, money(row.sales), money(Number(row.costs) - Number(row.employeeExpense || 0)), money(row.employeeExpense), money(row.utility), money(row.retentions), money(row.incomeTaxRetention), money(row.taxAfterRetentions), money(row.accumulatedBase), row.rate, money(row.tax)]));
    drawRow(['TOTAL ANUAL', money(detail.annual.sales), money(Number(detail.annual.costs) - Number(detail.annual.employeeExpense || 0)), money(detail.annual.employeeExpense), money(detail.annual.utility), money(detail.annual.retentions), money(detail.annual.taxRetentions), money(detail.annual.taxPayable), money(detail.annual.base), '', money(detail.annual.tax)]);
